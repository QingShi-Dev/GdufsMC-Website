...
export type NewLandmarkKind = "region" | "building" | "machine";

/**
 * 机器的输入/输出产物
 *  - icon 必填, 用图标地址作为唯一标识
 */
export interface NewLandmarkProduct {
  label: string;
  /** 图标地址 (/icons/.../xxx.png) */
  icon: string;
}
...
export type NewLandmarkWithLandmarksOverride = {
...
export interface NewLandmark {
  ...
  description?: string;
  /** 输入产物 (没填就不显示输入区) */
  inputs?: NewLandmarkProduct[];
  /** 输出产物 (没填就不显示输出区) */
  outputs?: NewLandmarkProduct[];
...
/** 按维度分组的地标列表 */
export type NewLandmarkGroups = Record<NewWorldId, NewLandmark[]>;
