 ...
 *
 * 注意: 站名/珍珠站已完全移到地标系统 (NewLandmark, 配 visibleWhen: "metro"),
 ...
 *  - 渲染时根据 NewGuideMap 算的 `landmarksVisible && metroVisible` 自动挑
 *
 ...
 */
export type NewMetroScale = number;
 ...
 */
export type NewMetroLabelFields = {
  fontSize?: number;
  fontSizeScale?: NewMetroScale;
  distance?: number;
  distanceScale?: NewMetroScale;
   ...
   * 但 NewMetroLabelFields 里也带上, 这样 withLandmarks 可以单独覆盖方向
   */
  direction?: number;
 ...
 */
export interface NewMetroLabelConfig extends NewMetroLabelFields {
   ...
   */
  withLandmarks?: NewMetroLabelFields;
}

/**
 * 站点公共字段
 *  - 站名已完全移入地标 (NewLandmark, 配 visibleWhen: "metro" 只在交通开时显示)
 *  - 这里只剩"画圆/胶囊"需要的图标信息
 */
export interface NewMetroStationBase {
  id: string;
  /** MC 世界坐标 (x, z) */
  x: number;
  z: number;
 ...
 */
export interface NewRegularStation extends NewMetroStationBase {
  kind: "regular";
  radius?: number;
  /** 半径的缩放系数 (默认 0.3) */
  radiusScale?: NewMetroScale;
}
 ...
 */
export interface NewTransferStation extends NewMetroStationBase {
  kind: "transfer";
  rotation?: number;
  length?: number;
  lengthScale?: NewMetroScale;
  width?: number;
  widthScale?: NewMetroScale;
}

export type NewMetroStation = NewRegularStation | NewTransferStation;
...
export interface NewLineStyle {
  ...
  cornerRadius?: number;
  /** 圆角缩放: 0=恒定, 1=跟k (默认) */
  cornerRadiusScale?: NewMetroScale;
}
...
export interface NewLine {
  ...
  nameConfig: {
    /** 起点端 */
    start: NewMetroLabelConfig;
    /** 终点端 */
    end: NewMetroLabelConfig;
  };
}
 ...
 *  - 继承 NewMetroLabelFields: fontSize/fontSizeScale/distance/distanceScale/offsetX/offsetY
 *    这 6 个字段是"标签通用"配置, 站名/线端/珍珠站共用同一组默认值
 */
export interface NewMetroStyleDefaults extends NewMetroLabelFields {
  /** 普通站 radius 默认 (CSS px@1x, 默认 4) */
  stationRadius?: number;
  /** 普通站 radius 缩放 (默认 0.3) */
  stationRadiusScale?: NewMetroScale;
  /** 换乘站 length 默认 (CSS px@1x, 默认 14) */
  stationLength?: number;
  stationLengthScale?: NewMetroScale;
  /** 换乘站 width 默认 (CSS px@1x, 默认 6) */
  stationWidth?: number;
  stationWidthScale?: NewMetroScale;
  /** 线路端 label 的"withLandmarks"字段覆盖 (fontSize / distance / offsetX / offsetY) */
  lineEndWithLandmarks?: NewMetroLabelFields;
  /** 线路线宽全局默认 (CSS px@1x, 跟 per-line 的 line.style?.width 同义) */
  lineWidth?: number;
  lineWidthScale?: NewMetroScale;
  lineWidthMax?: number;
  /** 线路圆角全局默认 */
  lineCornerRadius?: number;
  lineCornerRadiusScale?: NewMetroScale;
}

/**
 * 单一维度的地铁数据 (lines + stations 拆开放, 方便扩展)
 *  - 站名/珍珠站都已移入地标 (NewLandmark, 配 visibleWhen: "metro")
 *  - 这里只剩"画线 + 画站点图标"需要的几何/样式信息
 *  - style 字段是单维度全局默认值, 不填就走 BUILT_IN fallback
 */
export interface NewMetroDimension {
  lines: NewLine[];
  stations: NewMetroStation[];
  /** 单维度全局默认 (统一控制站标/字号/距离/线宽) */
  style?: NewMetroStyleDefaults;
}

/** 按维度分组 */
export type NewMetroGroups = Record<NewWorldId, NewMetroDimension>;

/* ============================== Defaults ============================== */

/** 站点本身 (icon) 何时显示: 缩放 >= 5.0x (500%)
 *  - 500% 以下只能看到线路, 看不到站点 (跟用户需求一致) */
export const METRO_STATION_MIN_ZOOM = 500;
