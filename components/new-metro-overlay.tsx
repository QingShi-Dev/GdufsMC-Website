 ...
 *
 *  1. NewMetroLines (SVG, 进 <g>) — 线路, viewBox 坐标
 ...
 *  2. NewMetroStations (HTML, 走 toScreen) — 站点 (圆 / 胶囊), 屏幕坐标
 *     - 大小用 CSS px, cssScaled 公式
 *
 * 站名/珍珠站已完全移到地标系统 (NewLandmark, 配 visibleWhen: "metro"),
 * 这里不再渲染站名文字, 只画站点图标。
 *
 * 线端 label (大数字方型) 在 NewGuideMap 里直接渲染 (不走 metro overlay),
...
import type {
  NewLine,
  NewMetroStation,
  NewMetroStyleDefaults,
  NewMetroLabelFields,
} from "@/lib/new-metro-types";
import { METRO_STATION_MIN_ZOOM } from "@/lib/new-metro-types";
...
export interface NewMetroLinesProps {
  lines: NewLine[];
  k: number;
...
export function NewMetroLines({ lines, k, toVB, defaults }: NewMetroLinesProps) {
...
export interface NewMetroStationsProps {
 ...
 */
export interface NewMetroLineEndLabelsProps {
  lines: NewLine[];
  k: number;
  currentZoom: number;
  isPanning: boolean;
...
export function NewMetroLineEndLabels({
  lines,
  k,
  currentZoom,
  isPanning,
